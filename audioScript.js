      ...
      for (let j = 0; j < landmarks.length; j++) {
        ...
        if (handedness === "Left") {
          if (previousPositionLeft) {
            speed = Math.sqrt(
              Math.pow(x - previousPositionLeft.x, 2) +
                Math.pow(y - previousPositionLeft.y, 2)
            );
          }
          ...
          if (previousPositionRight) {
            speed = Math.sqrt(
              Math.pow(x - previousPositionRight.x, 2) +
                Math.pow(y - previousPositionRight.y, 2)
            );
          }
        ...
        if (handedness === "Left" && !isPlayingLeft) {
          ...
          const note =
            scaleLeft[Math.floor((x / canvasElement.width) * scaleLeft.length)];
          synthLeft.volume.value = Tone.gainToDb(volume); // Set the volume
          synthLeft.triggerAttackRelease(note, duration);
          isPlayingLeft = true;
          setTimeout(() => {
            isPlayingLeft = false;
          }, 500); // Set the flag to false after the duration of the note
          ...
          const note =
            scaleRight[
              Math.floor((x / canvasElement.width) * scaleRight.length)
            ];
